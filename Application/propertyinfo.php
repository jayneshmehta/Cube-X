...
if ($con) {
  $data = $con->select("properties", "*", null, null, "property_id", $propid);
  $data = $data[0];
  ...
  $Facilitys = explode(",", $data['long_discription']);
}
...
<div class='row gx-0'>
  <div class="ps-5 pt-4 p-2">
    <nav aria-label="breadcrumb ">
      <ol class="breadcrumb  text-dark">
        ...
        <li class="breadcrumb-item text-secondary  " aria-current="page"><a href="<?php echo SITE_URL ?>PHPOPS/Application/showproperties.php" class='text-decoration-none text-secondary'>Properties</a></li>
        <li class="breadcrumb-item text-dark  active" aria-current="page">Propertie Info</li>
      </ol>
    </nav>
  </div>
  <div class='row d-flex justify-content-center' style='padding-inline: 100px;'>
    <div class='col-1 p-2  d-flex flex-column justify-content-center align-item-center'>
      ...
      foreach ($photos as $key => $value) {
        echo  "<img class='propertyprofilesrc my-2  p-1 border border-3 border-dark' width='100px' height='100px'  src='https://propertyimgbucket1.s3.ap-south-1.amazonaws.com/images/propertyImg/property" . $data['property_id'] . "/" . $value . "' alt='property img'>";
      }
      ?>
    </div>
    <div class='col-4 p-2 d-flex justify-content-center'>
      <img class='p-2 my-5 border border-3 border-dark align-items-center' id='propertyprofilesrc' width='500px' height='500px'  src='https://propertyimgbucket1.s3.ap-south-1.amazonaws.com/images/propertyImg/property<?php echo $data['property_id'] . "/" . $photos[0]; ?>' alt='property img'>
    </div>
    <div class='col-6'>
      <div class='display-5'><?php echo ucfirst($data['property_name']); ?></div> <br>
      ...
      <p class='ms-3 fs-3 '> <?php echo  number_format($data['price']);
                              if ($data['type'] == 'Rent') {
                                echo " /Month";
                              } else {
                                echo " Lacs";
  ...
  <div class='row d-flex' style='padding-inline: 300px;'>
    <div class='col-6 d-flex align-items-center justify-content-start ' id='ratingdiv'>
      ...
      <div class='col-3  d-flex align-items-center justify-content-start'>
        <select type='number' class='border border-3 border-warning rounded me-2  px-2 ms-3' id='propertyrating' <?php if (!$session) {
                                                                                                                    echo 'disabled';
                                                                                                                  } ?>>
  ...
  <div class='row mt-3' style='padding-inline: 300px;'>
    <div class='col-6 d-flex justify-content-center'>
      <button class="col-6 btn btn-primary fs-4" id='propertycontact_owner' <?php if ($session) {
                                                                              echo "data-bs-toggle='modal' data-bs-target='#contactdetails'";
                                                                            } else {
                                                                              echo "data-bs-toggle='modal' data-bs-target='#userlogin'";
                                                                            } ?> value="<?php echo base64_decode($_GET['id']); ?>">Contact owner</button>
    </div>
    <div class='col-6 d-flex justify-content-start'>
      <a href='<?php echo SITE_URL; ?>PHPOPS/Application/showproperties.php' class="col-6 btn btn-warning fs-4">See more properties </a>
    </div>
  </div>
